...
import { getDepartmentsAsync, getRolesAsync, getUsersAsync, updateUserAsync } from '../Redux/reducers/emsReducer';
    ...
    useEffect(()=>{
        ...
        return (
            ...
            <div className="flex flex-col justify-center  items-center px-10">
                <div className="grid"><p className=" text-black font-bold text-2xl mt-10" >Update Employee</p></div>
                <Formik 
                    initialValues={{fname:User.fname,lname:User.lname,gender:User.gender,age:User.age,phoneno:User.phone , role:User.role.name,department:User.department.name}}//fname, lname
                    validationSchema={schema}
                    onSubmit={(values, actions) => {
                        const idd =state1.employee.Departments.filter((val)=>{
                            if(val.name === values.department)
                            {
                                return val._id
                            }
                        });
                        ...
                        <form className="bg-gray-200 shadow rounded px-4 pt-6 pb-8  my-10 w-1/2 " onSubmit={handleSubmit}>
                            ...
                            text-gray-700 mb-3 leading-tight focus:outline-black"
                                ...
                                <datalist id="department">
                                    {
                                        state1.employee.Departments.map((val)=>(<option value={val.name}/>))
